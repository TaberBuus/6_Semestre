...
void setup_timer() {
  ...
  timerAlarmWrite(timer, 500000, true);          // set timer value to 500000 x 1 micro sec = 0.5 sec -> Fs = 2 Hz), true = reload
...
void loop() {
  ...
  if(doDataCollection){
    ...
    Serial.print(analogRead(strainSensorPin));     // write fsr value to serial 
    Serial.write(13);                           // CR
    Serial.write(10);                           // LF
    Serial.flush();
